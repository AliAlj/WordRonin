        ...
        for node in nodesAtPoint {
            if node.name == "enemy" {
                ...
                if let sprite = node as? SKSpriteNode,
                   let index = activeEnemies.firstIndex(of: sprite) {
                ...
                if let emitter = SKEmitterNode(fileNamed: "sliceHitBomb") {
                    emitter.position = node.parent?.position ?? .zero
                    addChild(emitter)
                }
                ...
                if let parent = node.parent as? SKSpriteNode,
                   let index = activeEnemies.firstIndex(of: parent) {
                    activeEnemies.remove(at: index)
                }
